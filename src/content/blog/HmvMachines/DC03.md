...
### enum4linux
`enum4linux`专门用于 **枚举 SMB / RPC 服务的 Windows 主机**
...
`172.16.55.212` 正在发送 `mDNS` 查询，例如 `A FileServer.local`， 这表明靶机一直在请求`FileServer` 的域名  
...
+ Account Operators 是 Active Directory (AD) 中的一个**本地域组**，其权限设计用于管理域内的用户、组和计算机账户，但权限范围存在特定限制。
+ 可以更改成员的密码！！！
+ **直接隶属组**：`Account Operators`（账户操作员）和 `Domain Users`（域用户）
